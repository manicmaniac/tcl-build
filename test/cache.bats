...
@test "packages are saved to download cache" {
  ...
  [ "$status" -eq 0 ]
  [ -e "${TCL_BUILD_CACHE_PATH}/package-1.0.0.tar.gz" ]
...
@test "cached package without checksum" {
  ...
  cp "${FIXTURE_ROOT}/package-1.0.0.tar.gz" "$TCL_BUILD_CACHE_PATH"
...
@test "cached package with invalid checksum falls back to mirror and updates cache" {
  ...
  touch "${TCL_BUILD_CACHE_PATH}/package-1.0.0.tar.gz"

  install_fixture definitions/with-checksum
  [ "$status" -eq 0 ]
  [ -x "${INSTALL_ROOT}/bin/package" ]
  [ -e "${TCL_BUILD_CACHE_PATH}/package-1.0.0.tar.gz" ]
  diff -q "${TCL_BUILD_CACHE_PATH}/package-1.0.0.tar.gz" "${FIXTURE_ROOT}/package-1.0.0.tar.gz"
...
@test "nonexistent cache directory is ignored" {
  ...
  export TCL_BUILD_CACHE_PATH="${TMP}/nonexistent"

  install_fixture definitions/without-checksum
  [ "$status" -eq 0 ]
  [ -x "${INSTALL_ROOT}/bin/package" ]
  [ ! -d "$TCL_BUILD_CACHE_PATH" ]
